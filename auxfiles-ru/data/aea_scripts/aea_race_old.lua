...
local genericResponse = {
	{format = false, text = "\"Все висело на волоске, рад, что мы остались в живых\""},
	{format = false, text = "\"Было бы чертовски обидно, если бы мы погибли там. И подумать только — это случилось бы, когда до спасения оставалось совсем чуть-чуть\""},
	{format = false, text = "\"После этого боя я уверен, что у Флагмана повстанцев нет никаких шансов!\""},
	{format = true, text = "Это, конечно, не та миссия, на которую подписался %s, но в любом случае он рад, что оказался здесь"},
	{format = true, text = "Вы находите %s за выпивкой с остальным экипажем, \"Что ж, даже если бы мы тогда погибли, думаю, это было бы не так уж плохо - ведь мы были вместе\""}
}
