...
script.on_render_event(Defines.RenderEvents.MAIN_MENU, function() end, function()
    ...
    Graphics.CSurface.GL_DrawRect(15, 520, 359, 185, Graphics.GL_Color(0, 0, 0, 0.8))
    Graphics.freetype.easy_print(10, 20, 525, "ВНИМАНИЕ: рекомендуется использовать FTLman \nвместо slipstream для патча ваших аддонов.")
    Graphics.freetype.easy_print(10, 20, 570, "Это позволит The Outer Expansion предоставлять \nсвои пользовательские системы кораблям, не \nотносящимся к OE.")
    Graphics.freetype.easy_print(10, 20, 635, "FTLman можно найти здесь: \nhttps://github.com/afishhh/ftlman/releases/latest\nЭто совершенно необязательно, если вам \nнеудобно переходить, игнорируйте это сообщение.")
end)
--[[
int iDamage;
int iShieldPiercing;
...
local warningText = "Вы находитесь внутри Кислотной туманности. Ваши сенсоры не будут работать, а пустые отсеки будут медленно и случайно пробиваться Кислотными облаками."
...
script.on_internal_event(Defines.InternalEvents.POST_CREATE_CHOICEBOX, function(choiceBox, event)
	...
	if event.eventName == "AEA_CULT_CON_ENTER_CODE" then
		goal_code = Hyperspace.playerVariables.aea_cult_code_goal
		choiceBox.mainText = "Ввод кода: "..code_entered
	elseif event.eventName == "AEA_CULT_CON_GENERATOR_CODE" then
		...
		choiceBox.mainText = choiceBox.mainText..goalString..". Вы быстро записываете его."
	end
end)
