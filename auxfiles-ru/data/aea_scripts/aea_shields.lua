...
local function get_level_description_aea_shields(systemId, level, tooltip)
    if systemId == Hyperspace.ShipSystem.NameToSystemId("aea_shields") then
    	if level == 2 then
        	return "Один уровень щита"
        elseif level == 4 then
        	return "Два уровня щита"
        elseif level == 6 then
        	return "Три уровня щита"
        elseif level == 8 then
        	return "Четыре уровня щита"
    	elseif level == 10 then
        	return "Пять уровней щита"
        elseif level == 12 then
        	return "Шесть уровней щита"
        elseif level == 14 then
        	return "Семь уровней щита"
        elseif level == 16 then
        	return "Восьмой уровень щита"
        elseif level%2 == 0 then
        	return string.format("%i уровень щита", math.floor(level/2))
        else
        	return ""
        end
    end
...
script.on_internal_event(Defines.InternalEvents.SHIP_LOOP, function(shipManager)
	if shipManager:HasSystem(Hyperspace.ShipSystem.NameToSystemId("aea_shields")) then
		...
		if shieldPower > normalCap and shipManager.shieldSystem.shields.power.first >= normalCap and shipManager.shieldSystem.shields.power.first + shieldExtra[shipManager.iShipId] < shieldPower then
			--print("CHARGE TIME")
			print("НАЧАЛО ЗАРЯДА:"..shipManager.shieldSystem.shields.charger)
			...
			print("КОНЕЦ ЗАРЯДА:"..shipManager.shieldSystem.shields.charger)
		...
		print("ТАЙМЕР:"..shieldTimer[shipManager.iShipId])
	end
end)
