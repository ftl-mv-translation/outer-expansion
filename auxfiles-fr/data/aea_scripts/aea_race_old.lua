...
local function get_level_description_aea_engines(systemId, level, tooltip)
    if systemId == Hyperspace.ShipSystem.NameToSystemId("engines") then
        if level > 8 then
        	if level%4 == 1 then
	            return string.format("Esquive: %i / FTL: %ix", 5*level - 5, math.floor(0.75 + level/4))
	        else
	            return string.format("Esquive: %i / FTL: %sx", 5*level - 5, tostring(0.75 + level/4))
	        end
        end
    end
end
...
local genericResponse = {
	{format = false, text = "\"Ça a été juste, je suis content que nous soyons sortis de là vivants.\""},
	{format = false, text = "\"Cela aurait été terriblement décevant si nous étions tombés là-bas. Et penser que ça aurait pu arriver alors que nous étions si proches de s'échapper.\""},
	{format = false, text = "\"Eh bien, après ce combat, je suis convaincu que le vaisseau amiral rebelle n’a aucune chance !\""},
	{format = true, text = "Eh bien, ce n’est certainement pas la mission à laquelle %s s’était inscrit, mais il est content d’être là malgré tout."},
	{format = true, text = "Vous trouvez %s en train de boire avec le reste de l’équipage, \"Eh bien, même si nous étions tombés là-bas, je ne pense pas que ça aurait été si terrible de le faire avec vous tous.\""}
}
