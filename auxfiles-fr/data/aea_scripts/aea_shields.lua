...
local function get_level_description_aea_shields(systemId, level, tooltip)
    if systemId == Hyperspace.ShipSystem.NameToSystemId("aea_shields") then
    	if level == 2 then
        	return "Un niveau de bouclier"
        elseif level == 4 then
        	return "Deux niveaux de bouclier"
        elseif level == 6 then
        	return "Trois niveaux de bouclier"
        elseif level == 8 then
        	return "Quatre niveaux de bouclier"
    	elseif level == 10 then
        	return "Cinq niveaux de bouclier"
        elseif level == 12 then
        	return "Six niveaux de bouclier"
        elseif level == 14 then
        	return "Sept niveaux de bouclier"
        elseif level == 16 then
        	return "Huit niveaux de bouclier"
        elseif level%2 == 0 then
        	return string.format("%i niveaux de bouclier", math.floor(level/2))
        else
        	return ""
        end
    end
...
script.on_internal_event(Defines.InternalEvents.SHIP_LOOP, function(shipManager)
	if shipManager:HasSystem(Hyperspace.ShipSystem.NameToSystemId("aea_shields")) then
		...
		if shieldPower > normalCap and shipManager.shieldSystem.shields.power.first >= normalCap and shipManager.shieldSystem.shields.power.first + shieldExtra[shipManager.iShipId] < shieldPower then
			--print("CHARGE TIME")
			print("DÉBUT DE LA CHARGE :"..shipManager.shieldSystem.shields.charger)
			...
			print("FIN DE LA CHARGE :"..shipManager.shieldSystem.shields.charger)
		...
		print("MINUTEUR :"..shieldTimer[shipManager.iShipId])
	end
end)
