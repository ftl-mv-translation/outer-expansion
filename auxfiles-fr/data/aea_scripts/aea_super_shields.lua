...
local function get_level_description_aea_super_shields(systemId, level, tooltip)
    if systemId == Hyperspace.ShipSystem.NameToSystemId("aea_super_shields") then
        if level%4 == 1 then
            return string.format("Couches: %i / Vitesse: %ix", 2 + math.ceil(level/2), math.floor(0.75 + level / 4))
        else
            return string.format("Couches: %i / Vitesse: %sx", 2 + math.ceil(level/2), tostring(0.75 + level / 4))
        end
    end
end
    ...
    if is_aea_super_shields(systemBox) then
        ...
        if activateButton.bHover then
            if Hyperspace.metaVariables.aea_super_shields_hotkey_enabled == 0 and not Hyperspace.ships.player:HasSystem(10) then
                Hyperspace.Mouse.tooltip = "Overclock du système de boucliers auxiliaires, vos super boucliers sont instantanément générés au maximum, puis le système est verrouillé pendant "..cooldownString.." secondes.\nRaccourci:C"
            else
                Hyperspace.Mouse.tooltip = "Overclock du système de boucliers auxiliaires, vos super boucliers sont instantanément générés au maximum, puis le système est verrouillé pendant "..cooldownString.." secondes.\nRaccourci:N/A"
            end
        end
...
mods.multiverse.systemIcons[Hyperspace.ShipSystem.NameToSystemId("aea_super_shields")] = mods.multiverse.register_system_icon("aea_super_shields")